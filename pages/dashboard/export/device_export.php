<?php 
	...
	$sql = "SELECT * FROM reader_devices,room WHERE d_location=r_id";
        ...
        <div class="col-12">		  
          <div class="card my-4 h-100" id="data_list">
			...
			<div class="card-header pb-0">
				<div class="d-lg-flex">
					<div>
						<h5 class="mb-0">Reader Devices List</h5>
						<p class="text-sm mb-0">
						List of all registered Reader Devices
						</p>
					</div>
					<div class="ms-auto my-auto mt-lg-0 mt-4">
					  <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
						  <div class="btn-group ms-1" role="group">
							<a class="btn btn-info btn-sm" href="javascript:void(0)" id="export_csv"   onclick="$('#table_device').DataTable().buttons(0,0).trigger()">CSV</a>
							<a class="btn btn-success btn-sm" href="javascript:void(0)" id="export_excel" onclick="$('#table_device').DataTable().buttons(0,1).trigger()">Excel</a>
							<a class="btn btn-primary btn-sm" href="javascript:void(0)" id="export_pdf"   onclick="$('#table_device').DataTable().buttons(0,2).trigger()">PDF</a>
						  </div>
						</div>
					</div>
				</div>
            ...
            <div class="card-body px-0 pb-2">
				<div class="row justify-content-center"> 
					<div class="table-responsive p-0 w-95">
					   ...
					   <table id="table_device" class="table table-hover table-fixed" style="width:100%">
							<thead>
								<tr>
									<th>No</th>
									<th>Device ID</th>
								...
								<tbody>
								    <?php
										$s_device = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
										$no =1;
										while ($d_device = mysqli_fetch_array($s_device)){
									?>
									   <tr>
											<td><?= $no;?></td>
											<td><?= $d_device['d_eui'];?></td>
											<td><?= $d_device['d_base'];?></td>
											<td><?= $d_device['r_name'];?></td>
											<td><?= $d_device['d_type'];?></td>
									   </tr>
				...
				buttons: [
					...
					},
					{
						...
						customize: function (doc) {
                           ...
                           doc.content[1].table.widths = [ '5%',  '30%', '25%', '25%', '15%'];
						   var now = new Date();
